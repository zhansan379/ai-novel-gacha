...
**难点：写真实题材容易漏查证、写反关系。** 中文职业/地域类题材（"山西下井""煤矿"这类）容易漏掉联网查证的环节；而前提一出现真实人物（如刘强东），又很容易把"主角资助他、他报恩共事"这类关系方向写反、把具体物件写变形。

**解法：开书先判"这本书需要什么资料"。** 按专业/历史/时效/设定连续性四维做一次判定，需要时就按中文专题联网预取、并入设定；正文只在时效性强的题材才临时联网。真实人物前提会补"早年求学"这类背景检索，并把"帮助→报恩→共事"这类关系方向固化为硬约束喂给正文，保证剧情方向端到端不出错。

**难点：长书越写越乱、也停不下来。** 越往后，世界观/角色/伏笔/关系的上下文越堆越多，模型既容易被带跑偏，也会一直写下去不收敛。

**解法：让模型"带着近期记忆"写。** 生成时只注入"最近几拍剧情 + 当下相关的角色伏笔"，避免上下文无限膨胀；加关系账本与知识图谱；每写一段就让 AI 判断这一章或整本书是否该收束，配单章段数上限兜底，防止无限续写。
...
| 文档 | 内容 |
|------|------|
| `docs/AI小说生成系统-需求文档.md` | 需求与验收基线（FR/NFR/优先级/指标） |
| `docs/AI小说生成系统-设计文档.md` | 总体架构 / 技术选型 / 核心流程 / 数据模型 |
| `docs/AI小说生成系统-接口契约.md` | API Spec（字段/类型/必填/取值范围/错误码） |
| `docs/AI小说生成系统-开发计划与质量门禁.md` | 里程碑 / RACI / 测试基线 |
| `docs/AI小说生成系统-工程规范与协作环境.md` | 编码规范 / 分支策略 / 环境 |
| `docs/AI小说生成系统-提示词与前置创作流程规范.md` | 提示词借鉴来源（MIT 合规）/ 世界观·历史·大纲前置流程 / 去AI味规则 |
| `docs/AI调用次数和花费说明.md` | 每次操作会调几次 AI、大概花多少钱的通俗估算 |
| `docs/DEPLOY.md` | 部署到公网访问的保姆级操作指引 |
| `docs/检索触发机制-开源对比与落地报告.md` | 真实信息召回触发机制的开源对比与落地设计 |
| `docs/小说生成开源项目调研报告.md` | 11 个 >200star 项目调研与优劣分析 |

> 以上文档均已改写成"大白话"版本，尽量少用系统内部代号和专有名词，方便非技术背景的人员阅读；技术细节（接口字段、错误码、命令等）仍然完整保留。
