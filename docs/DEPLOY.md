...
> **这份文档是干嘛的**：如果你第一次接触这个项目，只需要一台云服务器，照下面的步骤把项目跑起来、对外开放，让不同用户能注册、登录并各自使用。整个系统由 **FastAPI 后端 + Vue 前端 + SQLite** 组成，公网部署只需要一台云服务器 + Caddy（或 nginx）这一个转发工具 + 一个后端进程。
> 多用户之间互相隔离、以及让"每个用户自己带模型 Key（BYOK"）"的功能，代码里都已经做好了，不需要再另搭数据库或单独的登录认证服务。
...
下面这一条命令，是进入后端目录、建一个干净的运行环境并装上它需要的依赖（公网部署要用 `secure` 组，才能开启用户模型 Key 的加密）：
...
> **务必保持单进程跑后台程序**：`tasks`（异步开书任务）是进程内注册表，多 worker 会把任务打散。并发建议交给上一层（见下）。

## 三、前端构建 + Caddy（自动 HTTPS）

生产环境推荐 **Caddy 同域托管前端 + 反代 API**：浏览器只访问一个域名，`/v1/*` 交给后端，天然规避 CORS。

下面这段是 Caddy 的配置文件，落到服务器上后，把 `novel.example.com` 换成你自己的真实域名即可：
